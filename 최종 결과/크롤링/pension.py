...
w = driver.find_element(By.XPATH,'//*[@id="content"]/div[1]/section[2]/ul/li[4]/label')
...
time.sleep(1)
#------------------------------------------------------------
price = str(soup.select('div > div.price > p > b'))   # 가격
#price = soup.find_element(By.XPATH,'')   # 가격
price = re.sub('<.+?>', '', price)
price = price.replace(" ","")
price = price.replace("[","")
price = price.replace("]","")
price = price.replace(",","")
price = price.replace("다른날짜확인","0원")  # 다른날짜확인 , 판매완료는 어떻게 처리할지 의논
...
for i in range(leng):
    ...
    temp.append(txt[i])
    temp.append(loc[i])
    if len(good) <=i :   #애초에 5번째 평가가 없어서 오류남  (0 <= i < len(arr)) or (-len(arr) <= i < 0)
        good = []
        good.append(0.0) 
        temp.append(good[0])
    else:
    ...
    if length == 0:
        ...
        for j in range(length):  # 방의 갯수가 10개넘으면 ?
            temp.append(room_list[i][j])
            temp.append(pri_list[sum + j]+"원")
    sum = sum + length  
...
f = open(f'pension_{keyword}.csv','w',encoding='utf-8-sig',newline='')
writer = csv.writer(f)
...
for i in range(leng):
    ...
    urllib.request.urlretrieve(w_url, keyword+'/pension/image'+str(i)+ '번째'+'.jpg')
    ...
    if (length > 10 ):
        for j in range(10):
            ...
            urllib.request.urlretrieve(img_url, keyword+'/pension/pension'+str(i)+'번째 -'+str(j)+'.jpg')
    else:
